...
function AvatarUploader({ user, onUpdate, onError }: {
  user: User;
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async () => {
      setUploading(true);
      try {
        onUpdate(await userApi.uploadPicture(reader.result as string));
      } catch {
...
function ChangePasswordForm() {
  ...
  function setField(field: keyof typeof EMPTY_PW_FORM, value: string) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess(false);
    try {
      await userApi.updatePassword(form.current, form.next);
      setSuccess(true);
      setForm(EMPTY_PW_FORM);
    } catch {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Collapse in={open}>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField fullWidth label="Current Password" type="password" value={form.current} onChange={e => setField('current', e.target.value)} autoComplete="current-password" size="small" />
          <TextField fullWidth label="New Password" type="password" value={form.next} onChange={e => setField('next', e.target.value)} autoComplete="new-password" size="small" />
          <TextField fullWidth label="Confirm New Password" type="password" value={form.confirm} onChange={e => setField('confirm', e.target.value)} autoComplete="new-password" size="small" />
    ...
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', minHeight: '80vh', mt: 6, px: 2 }}>
      <Paper elevation={3} sx={{ width: '100%', maxWidth: 440, borderRadius: 3, overflow: 'hidden' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: '-52px', px: 3, pb: 3 }}>
          <AvatarUploader user={user} onUpdate={setUser} onError={setError} />

          <Typography variant="h5" sx={{ fontWeight: 700, mt: 1.5 }}>{user.username}</Typography>
          <RoleChip role={user.role} sx={{ mt: 0.75, letterSpacing: '0.05em' }} />
          ...
          <Box sx={{ width: '100%' }}>
            ...
            <Divider />
            <Field icon={<AdminPanelSettingsOutlinedIcon fontSize="small" />} label="ROLE" value={user.role.charAt(0).toUpperCase() + user.role.slice(1)} />
          </Box>
